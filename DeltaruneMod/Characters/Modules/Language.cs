...
namespace DeltaruneMod.Characters.Modules
{
    internal static class Language
    {
        ...
        private static void Language_collectLanguageRootFolders(List<string> obj) {
            string path = Path.Combine(Path.GetDirectoryName(DeltarunePlugin.Instance.Info.Location), "Language");
        ...
        public static void PrintOutput(string fileName = "") {
            ...
            if (!string.IsNullOrEmpty(fileName)) {
                string path = Path.Combine(Directory.GetParent(DeltarunePlugin.Instance.Info.Location).FullName, "Language", "en", fileName);
