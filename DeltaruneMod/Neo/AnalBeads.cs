...
{
    public class AnalBeads : ItemBase<AnalBeads>
    {
        public override string ItemName => "Bead Chain";

        public override string ItemLangTokenName => "ANAL_BEAD";
        ...
        private void CharacterMaster_OnInventoryChanged(On.RoR2.CharacterMaster.orig_OnInventoryChanged orig, CharacterMaster self)
        {
            ...
            var beadBehavior = player.GetComponent<AnalBeadBehavior>();
            if (itemCount > 0)
            {
                if (!beadBehavior)
                {
                    beadBehavior = player.gameObject.AddComponent<AnalBeadBehavior>();
        ...
        public class AnalBeadBehavior : CharacterBody.ItemBehavior
        {
            ...
            public event Action<AnalBeadBehavior> onDisabled;

            private DeployableSlot maxOrbsSlot = (DeployableSlot)8;

            public static int GetMaxProjectiles(Inventory inventory)
            {
                return baseMaxProjectiles + (maxProjectilesPerStack * inventory.GetItemCount(AnalBeads.instance.ItemDef));
            }
            ...
            public void InitializeOrbiter(ProjectileOwnerOrbiter orbiter, BeadProjectileController controller)
            {
                ...
                void DestroyOrbiter(AnalBeadBehavior beadBehavior)
                {
                    if (controller)
                    {
                        controller.Detonate();
            ...
            private void Awake()
            {
                ...
                //deploy.deployableSlot = maxOrbsSlot;
            ...
            private void FixedUpdate()
            {
                ...
                if (!body.master.IsDeployableLimited(DeployableSlot.LunarSunBomb) && projectileTimer > secondsPerProjectile)
                {
                    projectileTimer = 0f;
        ...
        public class BeadProjectileController : MonoBehaviour
        {
            ...
            private void AcquireOwner(ProjectileController controller)
            {
                ...
                if (player)
                {
                    ...
                    player.GetComponent<AnalBeadBehavior>().InitializeOrbiter(playerOrbiter, this);
                }
            }
