...
namespace DeltaruneMod.Items.Tier2
{
    public class CommRing : ItemBase<CommRing>
    {
        ...
        public override string ItemPickupDesc => "Gain healing and barrier on hit.";

        public override string ItemFullDescription => "On hit, heal <style=cIsHealing>5 hp</style> and gain <style=cIsHealing>5 temporary barrier</style>. <style=cStack>(+5 hp, +5 barrier, per stack)</style>." +
            "\nYou can heal a maxium of <style=cIsHealing>90% hp</style> and <style=cIsHealing>50% of hp as barrier</style>.";
        ...
        private void CommRingEffect(On.RoR2.GlobalEventManager.orig_OnHitEnemy orig, RoR2.GlobalEventManager self, RoR2.DamageInfo damageInfo, GameObject victim)
        {
            ...
            {
                var healing = itemCount * 5;
                if (!(body.healthComponent.health + healing > body.maxHealth * 0.9f)) body.healthComponent.health += healing;
                if (!(body.healthComponent.barrier + healing > body.maxHealth * 0.5f)) body.healthComponent.barrier += healing;
            }
        }
